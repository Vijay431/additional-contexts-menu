...
async function checkMarketplace(expectedVersion) {
  ...
  try {
    ...
    const postData = JSON.stringify({
      filters: [
        {
          criteria: [{ filterType: 7, value: 'VijayGangatharan.additional-context-menus' }],
        },
      ],
      flags: 914,
    });
    ...
    const marketplaceUrl =
      'https://marketplace.visualstudio.com/items?itemName=VijayGangatharan.additional-context-menus';
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      if (publishedVersion === expectedVersion) {
        ...
        console.log(
          `⚠️  Open VSX Registry: Version mismatch (expected: ${expectedVersion}, found: ${publishedVersion})`,
        );
