...
suite('Move Function to File - E2E Tests', () => {
  let testContext: Awaited<ReturnType<typeof E2ETestSetup.setup>>;

  suiteSetup(async () => {
    testContext = await E2ETestSetup.setup('moveFunctionToFile');
  ...
  suite('Command Registration', () => {
    test('should register Move Function to File command', async () => {
      const commands = await vscode.commands.getCommands();
      assert.ok(
        commands.includes('additionalContextMenus.moveFunctionToFile'),
        'Move Function to File command should be registered',
      );
    });
  });

  suite('Function Detection', () => {
    test('should move function when cursor is inside function', async () => {
      const testFile = path.join(testContext.tempWorkspace, 'move-function.ts');
      const content = `
const myFunction = () => {
...
`;
      ...
      const message = await vscode.commands.executeCommand(
        'additionalContextMenus.moveFunctionToFile',
      );
...
`;
  ...
  suite('File Selection', () => {
    ...
    test('should validate target file before moving', async () => {
      const sourceFile = path.join(testContext.tempWorkspace, 'source.ts');
  ...
  suite('Function Removal', () => {
    test('should remove function from source file after moving', async () => {
      const testFile = path.join(testContext.tempWorkspace, 'remove-function.ts');
...
`;
      ...
      assert.ok(!updatedContent.includes('toBeRemoved'), 'Function should be removed from source');
    });

    test('should preserve other functions in file', async () => {
      const testFile = path.join(testContext.tempWorkspace, 'preserve-others.ts');
      const originalContent = `
const removeMe = () => 'remove';
const keepMe = () => 'keep';
const alsoKeep = () => 'also keep';
`;
      await fs.writeFile(testFile, originalContent);
      ...
      assert.ok(updatedContent.includes('keepMe'), 'Other functions should be preserved');
      assert.ok(updatedContent.includes('alsoKeep'), 'Other functions should be preserved');
    });
  });

  suite('Import Handling', () => {
    test('should preserve function text when moving', async () => {
