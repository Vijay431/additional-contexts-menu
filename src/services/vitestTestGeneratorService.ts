...
/**
 * Service for generating Vitest test suites from function signatures
 */
export class VitestTestGeneratorService {
  private static instance: VitestTestGeneratorService | undefined;
  ...
  public static getInstance(): VitestTestGeneratorService {
    VitestTestGeneratorService.instance ??= new VitestTestGeneratorService();
    return VitestTestGeneratorService.instance;
  }
  ...
  public async generateTestsFromSelection(
    ...
    this.logger.info('Vitest test suite generated', {
  ...
  private generateImports(
    ...
    let imports = "import { describe, it, expect, beforeEach, afterEach, vi } from 'vitest';\n";
  ...
  private generateFunctionCall(
    ...
    const args = signature.parameters
      .map((p) => {
        ...
        const formatted = JSON.stringify(value, (_key, val) =>
          typeof val === 'function' ? '[Function]' : val,
        );
        // Replace mock function strings with vitest equivalents
        return formatted.replace(/"\[Function\]"/g, 'vi.fn()').replace(/"vi\.fn\(\)"/g, 'vi.fn()');
      })
      .join(', ');
  ...
  public async createTestFile(filePath: string, code: string): Promise<void> {
    ...
    this.logger.info('Vitest test file created', { filePath });
  }
  ...
  public async getTestGenerationOptions(): Promise<TestGenerationOptions | undefined> {
    ...
    const setupType = await vscode.window.showQuickPick(
      [
        ...
        {
          label: 'Basic',
          description: 'Basic beforeEach/afterEach with vi.clearAllMocks',
          value: 'basic',
        },
    ...
    // Ask about Vitest UI
    const includeVitestUi = await vscode.window.showQuickPick(
      [
        {
          label: 'Yes',
          description: 'Include Vitest UI comments for better test visualization',
          value: true,
        },
        { label: 'No', description: 'No Vitest UI comments', value: false },
      ],
      {
        placeHolder: 'Include Vitest UI comments?',
      },
    );

    // Ask about coverage comments
    const includeCoverageComments = await vscode.window.showQuickPick(
      [
        { label: 'Yes', description: 'Include coverage collection comments', value: true },
        { label: 'No', description: 'No coverage comments', value: false },
      ],
      {
        placeHolder: 'Include coverage comments?',
      },
    );
    ...
    return {
      ...
      customSetupPath,
      includeVitestUi: includeVitestUi?.value ?? false,
      includeCoverageComments: includeCoverageComments?.value ?? false,
      mockPatterns: [],
    };
  }
}
